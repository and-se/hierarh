...
def group_json_elements(input_file_path):
    try:
        ...
        episkop_data = list(itertools.chain.from_iterable(
                [data[i]['episkops'] for i in range(len(data))]))
        # print(episkop_data[0]['text'])

        with open('../output/episkops3.json', 'w', encoding='utf-8') as file:
            # json.dump(episkop_data, file, ensure_ascii=False, indent=4)
            for i in range(len(episkop_data)):
                try:
                    file.write(episkop_data[i]['text']+'\n')
                except TypeError:
                    file.write(episkop_data[i]+'\n')

        for index, item in enumerate(episkop_data):
...
input_file_path = '/tmp/mozilla_user0/articles_large_part3+.json'
